...
test-$1: test-prepare build-$1
	...
	$(DOCKER) run --rm -it --name $(IMAGE_NAME)-extra-$(shell cat $1/version.txt) -e POSTGRES_PASSWORD=postgres -p 5432:5432 -d $(REPO_NAME)/$(IMAGE_NAME)-extra:$(shell cat $1/version.txt) osm2pgrouting --version
	if [ "$(shell echo $1)" != "$(shell cat $1/version.txt)" ]; then\
		...
		$(DOCKER) run --rm -it --name $(IMAGE_NAME)-extra-$(shell echo $1) -e POSTGRES_PASSWORD=postgres -p 5432:5432 -d $(REPO_NAME)/$(IMAGE_NAME)-extra:$(shell echo $1) osm2pgrouting --version;\
	fi
endef
